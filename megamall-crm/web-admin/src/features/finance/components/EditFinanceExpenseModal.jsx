/**
 * EditFinanceExpenseModal — edit a business expense's amount/note with full
 * edit history. Category is fixed at creation and shown read-only.
 *
 * Props:
 *   expense   {object|null}  the row (must have id, amount, note, expense_category)
 *   onClose   {fn}
...
export default function EditFinanceExpenseModal({ expense, onClose, onSuccess }) {
  ...
  const { data: history = [], isLoading: histLoading } = useQuery({
    queryKey: ['finance-expense-history', expense?.id],
    queryFn: () => fetchFinanceExpenseHistory(expense.id),
    enabled: !!expense?.id,
  })
    ...
    >
      <div className="w-full max-w-[460px] rounded-2xl bg-white shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-[14px] font-bold text-slate-900">Редактировать расход</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Category — read-only, fixed at creation */}
          <div>
