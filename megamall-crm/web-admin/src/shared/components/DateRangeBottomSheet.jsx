/**
 * DateRangeBottomSheet — full-featured mobile date-range picker sheet: dd.mm.yyyy
 * text inputs, quick presets, weekday header, scrollable month grids and a
 * "Показать результаты — <range>" CTA that commits the draft on tap.
 *
 * Selections are staged in a local draft and only pushed to `onChange` when
 * the CTA is pressed (or immediately for the "Максимум" preset), matching the
 * pattern already used by PeriodRangeFilter's mobile sheet.
 *
 * Props:
 *   open      {bool}
 *   onClose   {() => void}
 ...
 *   title     {string}  sheet header (default 'Выбор периода')
 */
...
export default function DateRangeBottomSheet({ open, onClose, from = '', to = '', onChange, title = 'Выбор периода' }) {
