...
`internal/compensation` is the single source of truth for how much anyone has earned. Rates (`seller_rate`, `manager_team_rate`, `manager_personal_rate`, `team_lead_pool_rate`, `company_rate`) live in `CommissionConfig` rows scoped `global`/`team`/`employee` with `effective_from/to` windowing and employee→team→global fallback (`RateResolver`). Global defaults (10% / 3% / 20% / 40% / 60%, seeded in `migrations/00011_seed_commission_configs.sql`) are intentionally **not** owner-editable from the UI — changing them requires a new migration. Team/employee-level overrides ARE owner-editable, via `PersonalRateModal` on `web-admin/src/features/people/pages/TeamDirectoryPage.jsx` (`POST /hr/compensation/configs` with `scope: 'team'|'employee'`) — don't build a parallel rates UI. `OrderFinancialSnapshot` freezes the resolved rates at order-creation time for audit. Income reporting (`GetMyIncome`, `GetTeamIncome`, `GetTeamIncomeSummary`) aggregates `financial_events` by user/event type and already encodes the RBAC for "who can see whose income" — reuse it rather than re-querying `financial_events` directly from another module.
