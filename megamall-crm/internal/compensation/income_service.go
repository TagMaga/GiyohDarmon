...
// buildIncomeReportFromRows assembles IncomeReportResponse from pre-fetched data.
// Free function so income_test.go can unit-test the assembly logic without DB.
func buildIncomeReportFromRows(
	userID uuid.UUID,
	from, to time.Time,
	totalIncome float64,
	ordersCount int,
	aggRows []incomeAggRow,
...
// CheckIncomeAccess is the pure permission check, extracted for unit testing.
// hasOrderRelation is the result of the DB look-up (CanManagerAccessUser or
// CanTeamLeadAccessUser) — callers pass it in so the function stays DB-free.
func CheckIncomeAccess(
	actorID, targetID uuid.UUID,
	actorRole string,
	hasOrderRelation bool, // true = DB confirmed manager→seller or tl→member
) error {
	if actorID == targetID {
		switch actorRole {
		case "owner", "seller", "manager", "sales_team_lead":
