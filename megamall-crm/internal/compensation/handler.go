...
// ─── Delivery tariff handlers ─────────────────────────────────────────────────

// ListTariffs handles GET /hr/tariffs
// Returns a paginated list of all tariffs (active + historical).
func (h *Handler) ListTariffs(c *gin.Context) {
	p := pagination.ParseFromQuery(c)
	list, total, err := h.svc.ListTariffs(c.Request.Context(), p)
	if err != nil {
		response.HandleError(c, err)
		return
	}
	response.OKWithMeta(c, ToTariffResponseList(list), pagination.BuildMeta(p, total))
}

// GetActiveTariff handles GET /hr/tariffs/active
// Returns the single currently active tariff with its ranges.
func (h *Handler) GetActiveTariff(c *gin.Context) {
	t, err := h.svc.GetActiveTariff(c.Request.Context())
	if err != nil {
		response.HandleError(c, err)
		return
	}
	response.OK(c, ToTariffResponse(t))
}

// CreateTariff handles POST /hr/tariffs
// Creates a new tariff, deactivating the current one.
func (h *Handler) CreateTariff(c *gin.Context) {
	...
	t, err := h.svc.CreateTariff(c.Request.Context(), extractActor(c), req)
	if err != nil {
		response.HandleError(c, err)
		return
	}
	response.Created(c, ToTariffResponse(t))
}

// GetTariffByID handles GET /hr/tariffs/:id
func (h *Handler) GetTariffByID(c *gin.Context) {
	id, ok := parsePathUUID(c, "id")
	if !ok {
		return
	}
	t, err := h.svc.GetTariffByID(c.Request.Context(), id)
	if err != nil {
		response.HandleError(c, err)
		return
	}
...
// DeactivateTariff handles POST /hr/tariffs/:id/deactivate
// Closes an active tariff with a mandatory reason.
func (h *Handler) DeactivateTariff(c *gin.Context) {
	id, ok := parsePathUUID(c, "id")
	if !ok {
		return
	}

	var req DeactivateTariffRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, apperrors.BadRequest(err.Error()))
		return
	}
	...
	if err := h.svc.DeactivateTariff(c.Request.Context(), extractActor(c), id, req); err != nil {
		response.HandleError(c, err)
		return
	}
	response.NoContent(c)
